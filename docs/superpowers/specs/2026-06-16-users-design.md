...
**Поза межами:** історія замовлень (плейсхолдер; оживе в юніті orders), керування адресами (опційне — пропускаємо), скидання пароля поштою (ТЗ вимагає лише *зміну*). **Унікальність email** відкладена (потребує міграції): тут email обов'язковий при реєстрації, але не унікальний — перевірку унікальності зробимо в orders, де email стає ключем сповіщень.
